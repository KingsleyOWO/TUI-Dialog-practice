...
# 卸載 MariaDB（含完整清除命令）
uninstall_mariadb() {
    dialog --title "卸載 MariaDB" --yesno "警告：將刪除所有數據庫及相關檔案！確定繼續嗎？" 7 50
    [ $? -ne 0 ] && return

    {
        echo "20"; echo "# 停止服務..."; systemctl stop mariadb; systemctl disable mariadb
        echo "40"; echo "# 卸載 MariaDB 套件..."; apt-get purge --auto-remove -y mariadb-server mariadb-client libmariadb3 mariadb-client-core mariadb-common mariadb-server-core || yum remove -y mariadb-server mariadb
        echo "60"; echo "# 清除數據與配置檔..."; rm -rf /var/lib/mysql /etc/mysql /var/log/mysql /var/run/mysqld
        echo "80"; echo "# 清理不再需要的套件..."; apt-get autoremove -y || yum autoremove -y
        echo "100"
    } | dialog --title "卸載進度" --gauge "正在卸載 MariaDB..." 10 60 0

    dialog --title "完成" --msgbox "MariaDB 及其相關檔案已完全清除。" 5 40
}
